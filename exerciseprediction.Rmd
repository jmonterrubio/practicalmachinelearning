---
title: "Manner Exercise Prediction Using Measurement Body Devices"
...
mod1 <- train(classe ~ ., data=clean, trControl = train_control1, method="rpart")
...
confusionMatrix(predictions2, testing1$classe)$overall[1]
```

Good accuracy isn't it. Now we are goint to make a pair of researchs. First one 
...
And what happen if we remove some variable of the model.
...
We can see that just with the first 15 the error is already 0.0. So selecting the
...
mod3 <- randomForest(classe ~ roll_belt + yaw_belt + pitch_forearm + magnet_dumbbell_z +
                     ...
                     magnet_belt_y + accel_forearm_x + accel_dumbbell_y, importance=TRUE, data=training1)
predictions3 <- predict(mod3,newdata=testing1)
confusionMatrix(predictions3, testing1$classe)$overall[1]
```

Loosing a bit of accuracy but winning a lot of computation time.
